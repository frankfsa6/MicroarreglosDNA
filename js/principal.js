...
function creaBotones(pags) {
	...
	if(pags != 'login' && pags != 'joystick' && pags != 'numeros' && pags != 'chips')
	{
	...
	if(pags == 'numeros' || pags == 'chips'){
		...
		$("#tablaBotonesRutina").append("<th><button type='button' style='width:30vw; margin-left: auto; margin-right: auto;' class='btn btn-info btn-lg btn-block' id='codigoG' name='"+pags+"'>Có<u>d</u>igo G &nbsp;&nbsp;</button>");
		$("#tablaBotonesRutina").append("<a href='"+pags+".nc' download><button type='button' class='btn btn-md btn-light' id='codigoGlink' hidden></button></a><th>");
				...
				$.post("php/LSArch.php",{temporal:true}, function(resp){
					...
					$.post("php/LSArch.php",{rutinaIniciada:true}, function(datos){
						if(datos == '0'){
							pageKey = true;
							$("#tablaBotonesRutina").append("<td></td>");
							$("#tablaBotonesRutina").append("<td style='width:22%' style='width:20%'><button type='button' class='btn btn-success btn-lg btn-block' id='inicioProceso'><u>I</u>niciar proceso</button>");
							if(!$("#codigoG").length){
								$("#tablaBotonesRutina").append("<td style='width:2%></td>");
								$("#tablaBotonesRutina").append("<td style='width:22%' style='width:20%'><button type='button' class='btn btn-info btn-lg btn-block' id='codigoG' name='principal'>Có<u>d</u>igo G &nbsp;&nbsp;</button></td></tr>");
								$.ajax({
									type:'POST',
									url:'php/bd.php',
									data:{nombreRutina : 1},
...
function tecRap(event){
	...
	if( !( $(":text").length > 0 && $("#popup").attr("class") == "active" && navpag != "numeros" && navpag != "chips" ) && navpag != "login" && navpag != "joystick" ){
		switch(event.keyCode){
			// A: botón de anterior
			case 65:
...
function barraNav(){
	...
	if (navpag != "principal" && navpag != "logo" && ( navpag == actual || navpag == "joystick" || navpag == "login" || navpag == "numeros" || navpag == "chips" || pageKey) ){
		$("#espera").css({"width":"100%","height":"100%","cursor":"wait"});
		//Comprueba que la clase de joystick sea verdadera para eliminar la clase active
		$("#joystick, #login, #numeros, #chips").parent().removeClass("active");
	...
	// Recarga página del índice
		...
		if(joyconf == "joystick" || joyconf == "login" || joyconf == "chips" || joyconf == "numeros")
			$("li").removeClass("active");
		...
		if( existe ){
			...
			$("#botones").on("click","#codigoG", function(){
				...
				if(this.name == "principal"){
					...
					if(this.name != ""){
						$.post("php/RutinaG"+this.name+".php", function(){
							$("#codigoGlink").trigger("click");
						}).always(function(){
							$("#espera").css({"width":"0%","height":"0%","cursor":"auto"});
						});
